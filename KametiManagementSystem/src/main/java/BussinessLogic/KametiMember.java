...
static public KametiMember getAccount(HibernateDatabase hibr,int cnic)
{
 ...
 public void Deposite(HibernateDatabase hibr, double amount,int cnic,Transaction trans)
{
	 CriteriaBuilder builder = hibr.getSession().getCriteriaBuilder();
	    CriteriaQuery<KametiMember> criteria = builder.createQuery(KametiMember.class);
	    criteria.from(KametiMember.class);
	    ...
	    if(e.Member_CNIC==cnic)
	    {
	    	...
	    	KametiParty party=null;
					...
					try {
						...
						System.out.println("hello  " +e.Member_CNIC);
 ...
 public double Withdraw(HibernateDatabase hibr,int cnic,Transaction trans)
{
	 CriteriaBuilder builder =hibr.getSession().getCriteriaBuilder();
	    CriteriaQuery<KametiMember> criteria = builder.createQuery(KametiMember.class);
	    criteria.from(KametiMember.class);
    	...
    	if(e.getMember_cnic()==cnic)
    	{
    	    ...
    	    for (KametiParty k : entityList1) {
    	    	...
    	    	if(k.getKameti_ID()==kid)
    	    	{
    	    		 ...
    	    		 list1.add(k.getKameti_ID()+" "+k.getKametiName()+" "+k.getDates().getCreation_Date()+" "+k.getDates().getDeadline());
    	    	}
    	    }
