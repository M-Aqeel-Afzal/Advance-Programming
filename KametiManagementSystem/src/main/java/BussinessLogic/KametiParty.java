...
public void setMembers(KametiMember members,HibernateDatabase hibr) throws PartyCapecityIsFull, MemberAllreadyExist {
	...
	else
	{
	    ...
	    List<KametiParty> entityList = hibr.getSession().createQuery(criteria).getResultList();
...
static public void ShowParties(HibernateDatabase hibr)
{
...
static public ArrayList<String> getParties(HibernateDatabase hibr,int cnic)
{
	  // Create CriteriaBuilder
...
static public ArrayList<String> getPartyDetails(HibernateDatabase hibr,int kid)
{
	  // Create CriteriaBuilder
	ArrayList<String> list1=new ArrayList<>();
    CriteriaBuilder builder = hibr.getSession().getCriteriaBuilder();
...
static public ArrayList<String> getMembersDetails(HibernateDatabase hibr ,int kid)
{
  ...
  if(e.Kameti_ID==kid)
  {
	  ...
	  CriteriaBuilder builder1 = hibr.getSession().getCriteriaBuilder();
	    CriteriaQuery<KametiMember> criteria1 = builder1.createQuery(KametiMember.class);
	    criteria1.from(KametiMember.class);
	    List<KametiMember> entityList1 = hibr.getSession().createQuery(criteria1).getResultList();
	    for (KametiMember e1 : entityList1) {
	    	  if(e1.getKameti_ID()==e.Kameti_ID)
	    	  {
	    		  ...
	    		  CriteriaBuilder builder2 = hibr.getSession().getCriteriaBuilder();
	    		    CriteriaQuery<Accounts> criteria2 = builder2.createQuery(Accounts.class);
	    		    criteria2.from(Accounts.class);
	    		    List<Accounts> entityList2 = hibr.getSession().createQuery(criteria2).getResultList();
...
static void ShowPartiesofMember(HibernateDatabase hibr,int cnic)
{
	  // Create CriteriaBuilder
    CriteriaBuilder builder = hibr.getSession().getCriteriaBuilder();
    ...
    for (KametiMember e : entityList) {
    	if(e.getMember_cnic()==cnic)
    	{
    	    ...
    	    // Execute query
    	    List<KametiParty> entityList1 = hibr.getSession().createQuery(criteria1).getResultList();
